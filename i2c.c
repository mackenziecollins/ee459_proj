//i2c.c
// NOTE:
/* Currently, using the following methods to implement i2c have a 
 problem: when used to transmit multiple characters to a LCD screen in one 
 function call, some character will ended up missing. There's no telling 
 what caused this, but it could be caused by the transmittion speed difference 
 between Atmega328p and LCD screen. 

 Current solution is to instead decrease i2c speed to half (50 kHz), and only
 send one character at a time. (Or just a few?)
*/
